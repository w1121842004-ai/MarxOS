...
DEFAULT_INPUT = ROOT_DIR / "data" / "paragraph_cache_core.jsonl"
DEFAULT_OUTPUT = ROOT_DIR / "data" / "semantic_parent_cache_core.jsonl"
SPLIT_RE = re.compile(r"([^。！？；;!?]+[。！？；;!?]?)")


def _same_parent_group(left: dict, right: dict) -> bool:
    return all(
...
def _starts_new_parent(record: dict, current: list[dict], max_chars: int) -> bool:
    if not current:
        return False
    if not _same_parent_group(current[-1], record):
        return True
...
def _semantic_parent_record(records: list[dict], parent_index: int) -> dict:
    first = records[0]
    last = records[-1]
    text_parts = [
...
def build_semantic_parent_records(records: list[dict], max_chars: int) -> list[dict]:
    ...
    parent_index = 1
    for record in records:
        if _starts_new_parent(record, current, max_chars):
            parents.append(_semantic_parent_record(current, parent_index))
            parent_index += 1
            current = []
