...
当前阶段先暂停大规模结构重组，优先进入稳定化修复。原因是主链路已经具备基本分层，但数据切分、页码、检索路由、引用生成、Web follow-up 和部署入口仍需要先形成稳定契约。继续扩大重构范围会增加调试面，不利于把这一开发阶段收口。

本阶段的主计划以根目录 [`task.md`](../task.md) 为准。本文档保留为后续重构路线，不作为当前冲刺的首要执行清单。

稳定化阶段完成条件：

- 数据资产契约清楚：OCR cache、page map、paragraph cache、semantic parent cache、Milvus collection 的字段和来源可追溯。
- 检索行为可重复：典型篇目定位、概念解释、引文定位、专题追问和页码追问有固定回归用例。
- 生成输出可校验：回答中的引用注释必须能对应 evidence card，Web 展示字段稳定。
- 部署入口可复现：本地启动、健康检查、烟测、指标日志和依赖配置有明确步骤。
